...
app.component("travel", {
  ...
  computed: {
    startDate: {
      ...
      set: function () {
        console.log("tTime");
        offset =
          (new Date(this.startDate).getTimezoneOffset() / 60) * 3600000 * -1;
        this.endDate = new Date(
          new Date(this.startDate).getTime() + trajectTime + offset
        );
      },
    },
  },
  ...
  updated(){
    console.log("tTime");
    offset = (new Date(this.startDate).getTimezoneOffset() / 60) * 3600000*-1;
    this.endDate = new Date(
      new Date(this.startDate).getTime() + trajectTime + offset
    );
  },*/
  template: `
            ...
            <div id="demoMap" style="height: 500px; width: 750px"></div>
            <div class="listWay">
                List of your way
                <p v-if="ways.length == 0">No Way</p>
                <ul v-else>
            ...
            <form @submit.prevent="addTravel">
                ...
                Finish : <span v-if="endDate != null">
                {{new Date(new Date(endDate).getTime()+(new Date(startDate).getTimezoneOffset()/60 * 3600000)).toLocaleDateString("fr-FR",options)}}
                </span>
                <input type="submit">
            </form >
            </div>
  ...
  mounted() {
    ...
    d = new Date();
    console.log(d);
    console.log(new Date(d).getTimezoneOffset());
  ...
  methods: {
    choosePath: async function (index) {
      trajectTime = this.ways[index].trajectTime;
      console.log(this.ways[index]);
      console.log(trajectTime);
    ...
    addTravel: async function () {
      ...
      console.log(
        "DATE",
        new Date(this.year, this.month - 1, this.day, this.hour, this.minute)
      );
      ...
      newPaths = { name: this.trajectName, points: [], trajectTime };
      ...
      res = await fetch(
        "/api/peoples/search/linkPathPeople?idPeople=" +
          idPeople +
          "&idPath=" +
          idPath,
        {
      ...
      console.log(
        ...
        new Date(this.startDate).getTimezoneOffset()
      );
  ...
  methods: {
    ...
    addcarVue: async function () {
      let newCar = {
        ...
        format: this.Format,
        nbOfSeats: parseInt(this.nbOfSeats),
        owner: this.user._links.self.href,
      };
    ...
    travels: [
      {
        id: 1,
        slotName: "y",
        start: "2020-11-28T21:52:42.989982",
        end: "2020-11-28T22:07:42.990018",
        url: null,
        uid: null,
        lastModified: null,
        participants: [],
      ...
      },
      {
        id: 2,
        slotName: "s",
        start: "2020-11-28T21:52:42.990234",
        end: "2020-11-28T22:07:42.990241",
        url: null,
        uid: null,
        lastModified: null,
        participants: [],
